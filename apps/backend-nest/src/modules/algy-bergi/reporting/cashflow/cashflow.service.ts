...
export class CashflowService {
  ...
  async accounts(tenantId: string) {
    ...
    return rows.filter((row) => row.code).sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  async totals(
    ...
    const groupMap = new Map<
      string,
      { income: number; outcome: number; clients: Map<string, { code: string; name: string; income: number; outcome: number }> }
    >();
    for (const row of rows) {
      ...
      const client = group.clients.get(clientKey) ?? { code: row.clientCode, name: clientName, income: 0, outcome: 0 };
    ...
    const groups = Array.from(groupMap.entries())
      .map(([group, value]) => ({
        ...
        clients: Array.from(value.clients.values())
          .map((client) => ({ ...client, net: client.income + client.outcome }))
