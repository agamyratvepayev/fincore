...
export default async function BalanceSheetDetailsPage({ params, searchParams }) {
  ...
  const categoryName = String(
    totalsRows.find((row) => String(row.code ?? "") === String(category))?.label ??
      allRows[0]?.NAME ??
      `Category ${category}`
  ).trim();
  ...
  allRows.forEach((row) => {
    ...
    current.tmt += Number(row.AMOUNT ?? row.amount ?? 0);
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row, idx) => (
                  <tr key={`row-${idx}`}>
                    ...
                    <td className="income-details-money-col">{money(row.AMOUNT ?? row.amount)}</td>
