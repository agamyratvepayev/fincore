...
export async function fetchIncomeRevenueDetails(tenantId, filters = {}) {
  const url = withQuery(`/tenants/${tenantId}/reports/income-statement/details`, filters);
  const response = await fetch(url, { cache: "no-store" });
  ...
  if (!retryResponse.ok) throw new Error(`Failed to load revenue details (${retryResponse.status})`);
